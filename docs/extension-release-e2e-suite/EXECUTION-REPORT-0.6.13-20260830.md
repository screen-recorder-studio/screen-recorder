# Screen Recorder Studio 0.6.13 用户故事端到端执行报告
...
| 候选源码 commit | `b936b3a5a2c9e5449a08788a2d45babdf27993e1` |
| 构建路径/hash | `packages/extension/build` / `46f0b6de4ce4a4dfc4cba6282366d81da9ce0865308e0d38ea49b377400108cd` |
| 发布包 | `release-assets/0.6.13/screen-recorder-studio-v0.6.13.zip` |
| 发布包 SHA-256 | `8392d4dd803305ec6224ad7be239ed0b169f28dd24767ef240b4cb3eb3ee89b3` |
| Manifest 版本 | 0.6.13 |
...
| GATE-004 | 版本/Manifest/权限/声明审计 | PASS | 根包、扩展包、源与 build Manifest 均为 0.6.13；无 `host_permissions`；权限集合未扩大 |
| GATE-005 | `pnpm test:e2e:catalog` | PASS | 15 stories、54 cases；P0=32、P1=22、Smoke=27 |
| GATE-006 | Git/候选完整性 | PASS | 候选源码冻结为 `b936b3a`；`git diff --check` 通过；旧 0.6.12 资产与用户未跟踪文件未纳入候选包 |
...
- 其余 52 个 locale 未在本次补齐全部人工翻译，沿用 Chrome `default_locale=en` 回退策略。

## 5. 关键浏览器用例结果

| Case ID | 结果 | 实际结果 | 证据/限制 |
| --- | --- | --- | --- |
| AREA-001 | PASS | 360px Action 中 GIF 入口为语义 button，文案/版本 0.6.13 正常，无横向溢出 | Chrome 无障碍树 |
...
| 全量编辑 parity / WebM | NOT RUN | 未执行组合 Crop+Focus+Background 三格式 | 不声明 P1 Full |
...
- 判定：**灰度可发布**。
- 范围：Chrome Stable 152 / macOS arm64；0.6.13 候选包可上传 Chrome Web Store。
...
- 发布操作：0.6.13 ZIP 已上传并被 Chrome Web Store 解析为草稿；54 个语言和权限集合与已发布 0.6.12 一致。当前停在“提请审核”按钮前，等待操作时确认。
