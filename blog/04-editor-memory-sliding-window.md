# Web 视频剪辑引擎 (1)：内存虚拟化与滑动窗口机制

想象一下，你刚录制完一场精彩的 10 分钟 4K 游戏对局，兴冲冲地把这个 5GB 的视频文件拖进浏览器准备剪辑。进度条刚走到一半，屏幕突然一白，弹出了那张熟悉的、令人心碎的脸——**"Aw, Snap!" (崩溃啦)**。

这就是我们在开发 **Screen Recorder Studio** 时撞上的第一堵墙。

在浏览器中做视频剪辑，最大的敌人不是复杂的编码算法，而是**内存（Memory）**。传统的桌面软件（如 Premiere）可以直接读写硬盘，但在 Web 的沙盒里，如果我们试图将一个完整的长视频加载进内存（RAM），V8 引擎会毫不留情地通过 OOM（Out of Memory）教我们做人。

我们要实现的目标是“无限时长”录制与编辑。为了做到这一点，我们不能硬碰硬，而必须学会“欺骗”浏览器：**让它以为我们只在处理一小段视频，而实际上我们在操作一个庞大的数据集。**

这就是**内存虚拟化**与**滑动窗口**的故事。

> 🌟 **关于本项目**
> Screen Recorder Studio 是一个基于 Web 技术的开源视频录制与编辑工具。如果你对其中的技术实现感兴趣，欢迎访问我们的 [GitHub](https://github.com/screen-recorder-studio/screen-recorder) 或直接在 [Chrome 商店](https://chromewebstore.google.com/detail/screen-recorder-studio-fo/bondbeldfibfmdjlcnomlaooklacmfpa) 体验。

---

## 1. "滑动窗口" 架构设计

为了解决上述问题，我们设计了**滑动窗口（Sliding Window）**机制。

### 核心概念
想象你在漆黑的夜晚，拿着手电筒走过一条长长的画廊。
- **全局时间轴**：整条画廊（比如 1 小时的视频文件，静静躺在 OPFS 硬盘中）。
- **当前窗口**：手电筒照亮的区域（RAM 中实际加载的 2-4 秒视频块）。
- **视口（Viewport）**：用户在屏幕上看到的当前帧。

无论视频有多长，我们内存中永远只保留“手电筒”照亮的那一小部分数据。当用户拖动时间轴时，我们并不移动庞大的视频文件，而是移动那个轻量的“手电筒”。

```mermaid
graph LR
    Total[全局视频文件 (OPFS Disk)]
    Window[内存窗口 (RAM ArrayBuffer)]
    Player[播放器/解码器]

    Total -- "Seek 00:15" --> Window
    subgraph "滑动窗口 (Sliding Window)"
    Window -- "加载 [00:13 ~ 00:17]" --> Player
    end
    
    style Window fill:#f9f,stroke:#333,stroke-width:2px
```

### 状态定义
在 `@src/routes/studio/+page.svelte` 中，我们定义了驱动这一机制的核心状态：

```typescript
// 时间轴与窗口状态（毫秒）
let durationMs = $state(0);       // 视频总时长
let windowStartMs = $state(0);    // 当前窗口的起始时间
let windowEndMs = $state(0);      // 当前窗口的结束时间

// 全局帧索引
let globalTotalFrames = $state(0); // 总帧数
let windowStartIndex = $state(0);  // 当前窗口第一帧在全局的索引
```

## 2. 核心算法：智能窗口计算

如何决定窗口该“切”哪一部分？我们在 `computeFrameWindow` 函数中实现了这一逻辑。它不仅仅是简单的“前后各取 2 秒”，而是包含了一套基于用户行为的预测策略。

### 策略区分
1.  **Seek 模式（跳转）**：用户点击了时间轴的某一点。
    - 策略：以点击点为中心，前后各加载一部分（例如 `before=1.5s`, `after=1.5s`）。保证用户既能看到当前帧，也能立即进行小幅度的前后微调。
2.  **Play 模式（播放）**：视频正在连续播放。
    - 策略：**激进的后向加载**。我们不需要加载“过去”的帧，而是将资源全部用于加载“未来”的帧。检测到播放头接近窗口边缘时，静默触发下一个窗口的预取。

### 代码解析
这是 `computeFrameWindow` 的简化逻辑：

```typescript
function computeFrameWindow(params: FrameWindowParams): FrameWindowResult {
  const { centerMs, beforeMs, afterMs, fps, mode } = params;
  
  // 1. 计算目标帧索引
  const targetFrameIndex = Math.floor((centerMs / 1000) * fps);
  
  // 2. 根据模式调整窗口范围
  // 如果是预取 (prefetch)，我们只关心未来的数据
  const minWindowFrames = mode === 'prefetch' ? fps : fps * 2;
  
  // 3. 关键帧对齐 (Keyframe Alignment) - 极其重要！
  // 我们不能随意从第 N 帧开始切，必须找到第 N 帧之前的最近关键帧 (I-Frame)
  // 否则解码器会因为缺少参考帧而花屏
  let startFrame = targetFrameIndex - framesBefore;
  
  if (keyframeInfo) {
     // 回溯到最近的关键帧
     startFrame = findNearestKeyframe(keyframeInfo, startFrame);
  }

  return { startFrame, frameCount, ... };
}
```

## 3. 数据流与 Svelte 5 响应式驱动

Screen Recorder Studio 全面采用了 **Svelte 5 Runes** (`$state`, `$derived`, `$effect`) 来管理这套复杂的数据流。

### 为什么选择 Svelte 5？
在处理高频触发的视频事件（60fps 更新）时，React 的 Virtual DOM Diff 或 Vue 的依赖追踪可能会带来不必要的开销。Svelte 5 的细粒度响应式让我们能精确控制更新范围。

### 调度流程
1.  **触发**：`VideoPreviewComposite` 组件检测到需要数据（例如播放到了窗口边缘），回调 `onRequestWindow`。
2.  **计算**：主线程调用 `computeFrameWindow` 计算出新的 `startFrame` 和 `count`。
3.  **通信**：通过 `postMessage` 向 `opfs-reader-worker` 发送指令。
4.  **接收与替换**：
    Worker 从 OPFS 读取二进制数据并返回。主线程接收到数据后，**直接替换**整个 `workerEncodedChunks` 数组。

    ```typescript
    // opfs-reader-worker.ts 返回数据
    readerWorker.onmessage = (ev) => {
      if (type === "range") {
        // 核心：旧数组被丢弃，新数组接管。
        // JS 引擎的 GC 会负责回收旧数组的内存。
        workerEncodedChunks = chunks; 
        windowStartIndex = start;
        // ...更新状态，通知 UI 重绘
      }
    }
    ```

## 4. 性能优化的两个关键点

在实现滑动窗口时，我们发现了两个决定成败的性能细节。

### 4.1 内存策略："整窗替换" vs "追加缓冲"
在**数据层（Compressed Chunks）**，我们选择了**整窗替换**策略。
- **追加缓冲 (Ring Buffer)**：虽然听起来更“复用”，但在 Web Worker 间频繁传递小块数据并维护复杂的环形指针极易出错（Off-by-one error），且容易产生内存碎片。
- **整窗替换**：简单粗暴。每次窗口切换，旧的 `ArrayBuffer` 引用丢失，新的 `ArrayBuffer` 进场。
- **优势**：利用浏览器高效的垃圾回收（GC）机制，避免了手动内存管理的复杂性。只要我们不在此之外持有对旧 Chunk 的引用，内存就能始终保持在一个平稳的水位（例如始终只有 100MB）。

> **注**：在**解码层（VideoFrames）**，由于 `VideoFrame` 对象持有显存资源且必须手动 `close()`，我们在 Worker 内部依然维护了一个严格限制大小的队列（例如最多 150 帧），这与数据层的策略有所不同。

### 4.2 I/O 策略：批量读取 (Batch Read)
这是我们在性能分析中发现的 P0 级优化点。
最初，我们的逻辑是“要 100 帧，就循环 100 次 `FileHandle.slice()`”。结果发现拖动时间轴时 I/O 延迟巨大。

**优化后**：我们先计算窗口的字节范围。
1. 找到第 1 帧的 `offset`。
2. 找到第 100 帧的 `offset + size`。
3. **只执行一次** `blob.slice(start, end)` 读取整个 10MB 的二进制块。
4. 在内存中根据 `index.jsonl` 切割出 100 个小块。

```typescript
// src/lib/workers/opfs-reader-worker.ts
// ❌ Old: 100 IOPS
// chunks.forEach(c => file.slice(...))

// ✅ New: 1 IOPS
const totalSlice = file.slice(startOffset, endOffset); 
const totalBuf = await totalSlice.arrayBuffer();
// In-memory slicing...
```

这一改动将 I/O 耗时从 **50ms+** 降低到了 **2ms** 左右。

## 5. 总结与悬念

通过**滑动窗口机制**与**批量 I/O 优化**，Screen Recorder Studio 成功地将“对文件大小的依赖”转化为了“对时间切片大小的依赖”。
- 不管你录制了 10 分钟还是 10 小时，我们的编辑器内存占用始终保持在 **200MB 左右**。
- 配合 Svelte 5 的高性能调度，实现了媲美原生应用的丝滑剪辑体验。

**但是，解决了“存不下”的问题，紧接着就是“找不到”的问题。**

当用户点击时间轴上的 `05:20` 时，我们如何在几 GB 的文件中，瞬间找到那这确切的一帧？如果找错了位置，解码出的画面会不会花屏？

在下一篇，我们将深入“心脏”部位，揭秘**基于 OPFS 与 GOP 的帧级精准定位**技术。

---

> **上一篇**：(无)
> **下一篇**：[Web 视频剪辑引擎 (2)：基于 OPFS 与 GOP 的帧级精准定位](./05-editor-seeking-gop-alignment.md)