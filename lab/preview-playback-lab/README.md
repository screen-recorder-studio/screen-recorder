...
背压时允许“展示当前已完成帧”和“同时派发已合并的最新请求”并存。即使单帧合成超过一帧预算，候选也应降帧但持续更新，不能因为永远存在后继请求而把所有完成位图都关闭。
...
- 压力场景允许人工阻塞造成一次长帧，但最大显示间隔和最大时钟漂移不得超过“70ms 阻塞 + 34ms 恢复预算”；p95 时钟漂移仍不高于 34ms，且实际展示必须覆盖到 8 秒区间末尾。
- 精确定位探针在 0、静态区边界、静态区内部、恢复运动边界和末尾全部命中 last-frame-hold 应选的源帧。

`pressure` 场景会每 2 秒注入约 70ms 的 Worker 阻塞，目标不是消灭不可避免的单次长帧，而是确认候选在额外一个 34ms 帧预算内恢复、不排队追播过时位图，也不会在最后一次可观测样本后永久冻结。
