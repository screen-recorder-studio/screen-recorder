<script lang="ts">
  ...
  function onSliderInput() {
    ...
    debounceTimer = setTimeout(() => { requestFrame(idx) }, 100)
  }
  ...
  function play() {
    ...
    playbackTimer = setInterval(() => {
      if (!playing) return
      if (currentIndex >= totalFrames - 1) { pause(); return }
      currentIndex = Math.min(totalFrames - 1, currentIndex + 1)
      requestFrame(currentIndex)
    }, interval)
  }
  function pause() { playing = false; if (playbackTimer) { clearInterval(playbackTimer); playbackTimer = null } }
  function togglePlay() { playing ? pause() : play() }
  function step(delta: number) { pause(); currentIndex = Math.max(0, Math.min(totalFrames - 1, currentIndex + delta)); requestFrame(currentIndex) }
