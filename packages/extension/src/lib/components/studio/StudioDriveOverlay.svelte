<script lang="ts">
  ...
  import { X, HardDrive, ExternalLink, Trash2, LoaderCircle, Play } from '@lucide/svelte'
  ...
  async function loadThumbnails() {
    for (const rec of recordings) {
      ...
      thumbnails[rec.id] = { url: null, loading: true, error: false }

      try {
        // Try cached first
        const cached = await readCachedCover(rec.id)
        if (cached) {
          thumbnails[rec.id] = { url: cached, loading: false, error: false }
          continue
        }
        // Generate
        const url = await generateThumbnail(rec)
        thumbnails[rec.id] = { url, loading: false, error: !url }
      } catch {
        thumbnails[rec.id] = { url: null, loading: false, error: true }
      }
    }
  }
...
>
  ...
  <!-- Floating panel -->
  <div class="relative bg-white rounded-2xl shadow-2xl flex flex-col animate-overlay-in
